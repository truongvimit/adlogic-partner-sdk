...
AdLogic allows ad requests to be attempted when UMP reports an error or the network timeout elapses (20 seconds by default), including on the first launch. This is the SDK's own fallback: it does not record consent as granted, it does not add `npa`, and it does not guarantee fill. A form already on screen still waits for the user to answer. The fallback does not persist across a new process; each new UMP call starts a new wait unless UMP already allows requests. A host that turns requests off (`OnboardingSdk.setCanRequestAds(false)`, or `ConsentCenter.setHostConsent(false, false)` from its own CMP) still takes priority. This behavior differs from Google's guidance of using `ConsentInformation.canRequestAds()` alone as the gate.
