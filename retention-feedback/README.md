...
The SDK Activity treats a resumed but temporarily unfocused or host-blocked window as waiting, while retaining the same durable session and original ENTRY/PROMPT purpose. It retries on focus and core UI readiness signals; a short 250 ms poll covers the first 10 seconds, then stops. A 60-second monotonic deadline bounds each uninterrupted wait and survives rotation. These are SDK defensive timing choices. Timeout cancels the session and emits `retention_feedback_failed` with `reason=ui_readiness_timeout`; `shown` is emitted only after a valid lease is acquired, once per session. Config disable, shutdown and terminal sessions close the waiting Activity. Pause releases its lease; finish/destruction remove its readiness callbacks/subscription. An explicit uninstall survey remains available to paid users under the same host UI gates. Partners do not add a focus retry loop.
