package com.itg.customactivitycrash;
...
import com.itg.customactivitycrash.activity.ITGErrorActivity;
...
public final class CustomActivityOnCrash {
    ...
    private static final String EXTRA_CONFIG = "com.itg.customactivitycrash.EXTRA_CONFIG";
    private static final String EXTRA_STACK_TRACE = "com.itg.customactivitycrash.EXTRA_STACK_TRACE";
    private static final String EXTRA_ACTIVITY_LOG = "com.itg.customactivitycrash.EXTRA_ACTIVITY_LOG";
    private static final String EXTRA_CUSTOM_CRASH_DATA = "com.itg.customactivitycrash.EXTRA_CUSTOM_CRASH_DATA";

    //General constants
    private static final String INTENT_ACTION_ERROR_ACTIVITY = "com.itg.customactivitycrash.ERROR";
    private static final String INTENT_ACTION_RESTART_ACTIVITY = "com.itg.customactivitycrash.RESTART";
    private static final String CAOC_HANDLER_PACKAGE_NAME = "com.itg.customactivitycrash";
    ...
    private static ITGCrashConfig config = new ITGCrashConfig();
                    ...
                    Thread.setDefaultUncaughtExceptionHandler((thread, throwable) -> {
                        if (config.isEnabled()) {
                            ...
                            if (hasCrashedInTheLastSeconds(application)) {
                                ...
                                Log.d(TAG, "backgroundMode: " + config.getBackgroundMode() + " (SHOW_CUSTOM=" + ITGCrashConfig.BACKGROUND_MODE_SHOW_CUSTOM + ")");
                                ...
                                } else if (config.getBackgroundMode() == ITGCrashConfig.BACKGROUND_MODE_SHOW_CUSTOM || !isInBackground
                                ...
                                } else if (config.getBackgroundMode() == ITGCrashConfig.BACKGROUND_MODE_CRASH) {
                                    if (oldHandler != null) {
                                        oldHandler.uncaughtException(thread, throwable);
                                        return;
                                    }
     ...
     */
    @Nullable
    public static ITGCrashConfig getConfigFromIntent(@NonNull Intent intent) {
        ITGCrashConfig config = (ITGCrashConfig) intent.getSerializableExtra(CustomActivityOnCrash.EXTRA_CONFIG);
     ...
     */
    public static void restartApplicationWithIntent(@NonNull Activity activity, @NonNull Intent intent, @NonNull ITGCrashConfig config) {
    ...
    public static void restartApplication(@NonNull Activity activity, @NonNull ITGCrashConfig config) {
     ...
     * @param config   The config object as obtained by calling getConfigFromIntent.
     */
    public static void closeApplication(@NonNull Activity activity, @NonNull ITGCrashConfig config) {
        if (config.getEventListener() != null) {
            try {
    ...
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    @NonNull
    public static ITGCrashConfig getConfig() {
        return config;
    }
     ...
     */
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public static void setConfig(@NonNull ITGCrashConfig config) {
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="com.itg.customactivitycrash.RESTART" />,
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="com.itg.customactivitycrash.RESTART" />,
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="com.itg.customactivitycrash.ERROR" />,
    ...
    private static Class<? extends Activity> guessErrorActivityClass(@NonNull Context context) {
        ...
        if (resolvedActivityClass == null) {
            resolvedActivityClass = ITGErrorActivity.class;
        }
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="com.itg.customactivitycrash.ERROR" />,
     * If there is no activity with that intent filter, this returns null.
     *
     * @param context A valid context. Must not be null.
     * @return A valid activity class, or null if no suitable one is found
