...
Ticket 01 owns this contract. Package: `io.retentionkit.core`. The signatures below compile as of the core implementation commit; downstream code depends only on this contract, not the reference apps. Android minSdk 24, compileSdk 36, JVM 17. `retention-core` depends on AndroidX lifecycle-process/core only; no ads, Firebase, Compose, review or billing. The other library modules currently contain publication scaffolds, not feature implementations.
...
- `runtime.signal(signal): Boolean` means accepted for dispatch (false after shutdown), not successful completion of every subscriber. Durable user-state changes commit before observers receive the signal; a storage failure is diagnosed and that signal is not delivered.
...
- `runtime.config: RetentionConfigSnapshot` has immutable `revision: Long`, `values: Map<String,String>` and string/long/boolean helpers. Initial overrides merge below persisted values; module defaults apply only when a key is absent.
- `runtime.updateConfig(overrides, removeKeys = emptySet()): RetentionConfigResult` performs **PATCH**, not replacement. It validates the complete resulting profile with every installed module's `validateConfig`, synchronously stores one whole revision, publishes it, then emits ConfigurationChanged. Invalid values/validator exception reject the entire update and preserve the previous revision. Removal is explicit; absent remote fields never erase defaults.
