...
**Historical build validation:** all six project and six POM-only Maven release builds passed R8/resource shrinking at `bf68f1e`. The corrected Splash front door and SDK interfaces need fresh affected checks; the old matrix is not evidence for this source. See [VERIFICATION.md](VERIFICATION.md) for exact source/version and evidence. This application remains an unpublished local packaging proof.
...
- Feedback profile opens its optional exit-feedback UI through Splash; automatic feedback shortcut creation is disabled here. Continue opens Android’s uninstall confirmation; test by cancelling it. No completed uninstall is inferred from opening a system screen.
...
The final corrected source69d0f71 passed all12 project/POM-only release R8/runtime graph/manifest/composition checks after six fresh local publications at exact QA version `retentionkit-qa-20260907-69d0f71`. The core-project proof consumer passed9 fresh actual Activity tests within the279-case correction invocation. [Matrix](/Users/Shared/Panacea/Documents/SDKOptimize/retentionkit-correction/evidence/consumers-69d0f71/matrix-summary.json) · [Fresh tests](/Users/Shared/Panacea/Documents/SDKOptimize/retentionkit-correction/evidence/final-units-69d0f71/result.json). See [VERIFICATION.md](VERIFICATION.md) for current provenance and clearly separated historical records.

The full example separately passed17 physical Pixel/API34 cases and genuine API36 first-install onboarding through the standard ad/Main chain. Consumer packaging does not certify every flavor's device behavior. Final app R8, OS/ad limits, restoration/cleanup and PR status are in [the correction ledger](../.scratch/retentionkit/correction-acceptance.md). Local QA coordinates remain unreleased remotely.
