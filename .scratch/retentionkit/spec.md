...
5. Reusable exit-feedback UI: optional reasons, feature rescue/Keep, clear Continue to Android uninstall confirmation, with an explicit app-management option/fallback. No mandatory survey, no uninstall interception, no success claim after opening system Settings.
...
- Common-plan slots are daily 08:00/19:00, winback 11:00/14:00 and lockscreen 11:30/17:00/20:00; the new-user cohort is two elapsed days. The canonical PDF sets winback dormancy of 14–45 days and a lifetime maximum of 3 submissions. Post-setup marketing grace of 24 hours is an explicit SDK default. The old SDK 48-hour winback behavior remains only in the explicitly selected LEGACY_SDK preset; it was not a verified Translate condition. Validated per-key overrides remain supported.
- Onboarding abandonment is an explicit eligibility phase: it requires active **unfinished** onboarding, so it must not pass through an unconditional setup-completed gate. The common plan uses 24 hours of install-based onboarding grace (the explicit legacy preset uses zero), and its background confirmation delay is 3 seconds; it still requires known non-subscriber state, permission/channel, no external transition, current background, TTL/cap/cooldown and active campaign. Completing/aborting onboarding or returning foreground cancels it. The 24-hour post-setup grace remains the default for ordinary after-setup marketing. A configurable nonzero onboarding grace is measured from install; do not reset it on every onboarding screen.
...
Final evidence:549 unit/Robolectric cases with explicit split provenance,15/15 actual API36 emulator cases,12/12 project/POM-only Maven R8 consumers,12 runtime graph/composition checks,6 publication inspections and full example debug/test/release builds. Actual manual checks cover launcher widget lifecycle, feature/feedback/Store return, permissions/Later/shortcuts, reboot/timezone and force-stop relaunch. At that checkpoint the final debug APK was installed behind the Pixel5 PIN. The device is now unlocked: ticket09 records15/15 Android cases and additional manual checks, with full exclusive-device acceptance still pending after concurrent input; baseline and the scoped physical Home/return test remain separately recorded. No universal OEM/Play guarantee is inferred.
...
## Reopened default notification request — 2026-09-07

User observes no notifications in ordinary example use. New base2865f6d; tickets15–18 supersede previous acceptance for default notification behavior, screen wake and integration simplicity. Follow original Noti docs, including immediate quiet Reminder after completed setup; original first-day unfinished-setup rule is not a24h delay after setup. Device validation must use real default runtime and OS alarms, separately labelled from QA.
