...
object ExampleQa {
    ...
    fun prepare(application: Application, setup: Boolean = true, extra: Map<String, String> = emptyMap(), clear: Boolean = true,
        ...
        // Actual Onboard state/terminal remain authoritative even when initial fixture setup is false.
        return kit
    }
    ...
    fun attach(activity: Activity, parent: LinearLayout) {
        ...
        action("QA: pending setup · real Onboard state remains authoritative") { prepare(activity.application, setup = false); activity.recreate() }
