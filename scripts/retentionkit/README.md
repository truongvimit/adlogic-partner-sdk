...
Profiles: `core`, `notifications`, `widgets`, `feedback`, `review`, `umbrella`. Each selective profile permits itself plus core; unrelated retention modules, umbrella `retentionkit`, ads/MMP vendors, Firebase, Compose, billing/paywall, and OnboardKit are rejected by named coordinate/package rules. The `umbrella` profile permits all RetentionKit modules and requires the real `retentionkit` POM; it still rejects the unrelated vendor/suite stack. Core/review/feedback also reject WorkManager. Profiles other than notifications/umbrella reject notification/boot/wake-lock permissions. Only notifications and umbrella permit `WAKE_LOCK`, for screen-wake requests (requested lease deadline≤20s, maximum two attempts per message). An actual background alarm retains its framework receipt through confirmation≤2s and one lease (overall≤23s owner deadline); independent raw cleanup precedes receipt finish. No standalone partial lock substitutes for the component lifetime. This is not a hard real-time or physical screen-off guarantee. All profiles still reject foreground-service, exact-alarm, full-screen-intent, overlay, and battery-exemption permissions. Exported receivers whose names indicate debug/test are rejected. This is a focused leakage check, not a complete manifest security audit.
