...
Android supplies no negative pin callback. Timeout (default 120 seconds), a background→foreground return without confirmation, or a disable operation produces `Unknown(token, reason)`. These do not claim cancellation or installation. Dismissing the SDK invitation itself emits `retention_widget_invitation_dismissed` and never calls the launcher. Pin results/events have no Activity reference or host callback to retain across lifecycle changes.
